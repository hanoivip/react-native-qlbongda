...
-- 屏蔽玩家发言
url.friends.block = "friend/block"
url.friends.blockList = "friend/blockList"
...
url.fancyCard.incorporate = "fancyCard/incorporate" --梦幻碎片合成

--公会BOSS
url.guildBoss = {}
url.guildBoss.buyBuff = "guildBossWar/buyBuff" --购买buff 参数1:buffId  参数2:season
url.guildBoss.getTeam = "guildBossWar/getTeam"
url.guildBoss.saveTeam = "guildBossWar/saveTeam"
url.guildBoss.clearTeam = "guildBossWar/clearTeam"
url.guildBoss.info = "guildBossWar/info"
url.guildBoss.start = "guildBossWar/start"
url.guildBoss.detail = "guildBossWar/detail" --挑战详情
url.guildBoss.damageRank = "guildBossWar/damageRank" --伤害排行
url.guildBoss.bossCoinRecord = "guildBossWar/bossCoinRecord" --冠军币日志
...
-- 屏蔽玩家发言
function req.friendsBlock(pid, sid, isBlock, oncomplete, onfailed)
    local data = {
        pid = pid,
        sid = sid,
        isBlock = isBlock
    }
    return req.post(url.friends.block, data, oncomplete, onfailed)
end

-- 查看屏蔽列表
function req.friendsBlockList(oncomplete, onfailed)
    return req.post(url.friends.blockList, nil, oncomplete, onfailed)
end
...
function req.fancyCardIncorporate(fancyCardId, oncomplete, onfailed)
    local data = {
        cardId = fancyCardId
    }
    return req.post(url.fancyCard.incorporate, data, oncomplete, onfailed)
end
...
function req.guildBossBuyBuff(id, season, oncomplete, onfailed)
    local data = {
        buffId = id,
        season = season,
    }
    return req.post(url.guildBoss.buyBuff, data, oncomplete, onfailed)
end

function req.guildBossSaveTeam(nowTeamId, nowFormationId, initPlayersData, replacePlayersData, teamType, keyPlayersData, tacticsData, selectedType, isDefaultFormation, oncomplete, onfailed)
    local data = {
        ptid = nowTeamId,
        formationID = nowFormationId,
        init = initPlayersData,
    ...
    return req.post(url.guildBoss.saveTeam, data, oncomplete, onfailed)
end

-- 一键清空阵容
function req.guildBossTeamClear(nowTeamId,teamType,oncomplete,onfailed)
    local data = {
        ptid = nowTeamId,
        teamType = teamType
    }
    return req.post(url.guildBoss.clearTeam, data, oncomplete, onfailed)
end

function req.guildBossGetTeam(oncomplete, onfailed)
    return req.post(url.guildBoss.getTeam, nil, oncomplete, onfailed)
end

function req.guildBossInfo(oncomplete, onfailed)
    return req.post(url.guildBoss.info, nil, oncomplete, onfailed)
end

function req.guildBossMatch(season, bossId, oncomplete, onfailed)
    local data = {
        season = season,
        bossId = bossId,
    }
    return req.post(url.guildBoss.start, data, oncomplete, onfailed)
end

function req.guildBossDetail(oncomplete, onfailed)
    return req.post(url.guildBoss.detail, nil, oncomplete, onfailed)
end

function req.guildBossDamageRank(season, bossId, oncomplete, onfailed)
    local data = {
        season = season,
        bossId = bossId,
    }
    return req.post(url.guildBoss.damageRank, data, oncomplete, onfailed)
end

function req.guildBossWarBossCoinRecord(oncomplete, onfailed)
    return req.post(url.guildBoss.bossCoinRecord, nil, oncomplete, onfailed)
end