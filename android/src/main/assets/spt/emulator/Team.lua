...
local QuestConditionConfig = import("./QuestConditionConfig")
...
function Team:ctor(match)
    ...
    -- 球场上对应位置的额外全属性加成
    self.roleInitBuffBonus = nil

    self.passTimesBeforeGoal = 0
    self.skillCount = {}
    self.skillCountByType = {}
    self.skillPlusEffectCount = {}
    self.statisticsByPosition = {}
    self.goalCountByGoalWay = {}
    self.statisticsByBaseId = {}
    self.goalInfoList = {}
    self.passInfoList = {}
    -- self.starterCountByNation = {}
    -- self.starterCountByLeague = {}
...
function Team:initTeamAthletes(enemyTeam, teamInitializer, startId, startOnfieldId)
    ...
    for uid, _athlete in ipairs(teamInitializer.athletes) do
        ...
        if teamInitializer.cardBuff ~= nil then
            local buffBounsValue = teamInitializer.cardBuff[tostring(uid)]
            if buffBounsValue ~= nil then
                athlete.initBuffBonus = {value = buffBounsValue / 100}
            end
        end
        ...
        -- self.statisticsByBaseId[athlete.baseId] = {
        --     goal = 0,
        --     assist = 0
        -- }
...
function Team:initTeamStates(teamInitializer)
    ...
    self:initRoleInitBuffBonus(teamInitializer)
    self:initSkillCount()
    self:initSkillCountByType()
    self:initStatisticsByPosition()
    self:initGoalCountByGoalWay()
    self:updateDcInfo()
    self:assignTeamRole()
...
function Team:initRoleInitBuffBonus(teamInitializer)
    if teamInitializer.fieldBuff == nil then
        return
    end

    self.roleInitBuffBonus = {}

    for k, v in pairs(teamInitializer.fieldBuff) do
        self.roleInitBuffBonus[k] = {value = v / 100}
    end
end
    ...
    for i, athlete in ipairs(self.athletes) do
        ...
        if chargeTrumpetSkill and selector.tossCoin(chargeTrumpetSkill.probability) then
            ...
            if chargeTrumpetSkill.level > maxLevel then
                ...
                maxLevelCaster = athlete
            end
        end
    end

    if maxLevelBuff then
        if maxLevelSkill:isTypeOf(Skills.ChargeTrumpetEx1) then
            self.enemyTeam:judgeChargeTrumpetEx1Skill(maxLevelCaster, maxLevelSkill)
        end
        for j, selfAthlete in ipairs(self.athletes) do
            selfAthlete:addBuff(maxLevelBuff, maxLevelCaster)
...
function Team:judgeFightTogetherSkill()
    ...
    if maxLevelBuff then
        for j, selfAthlete in ipairs(self.athletes) do
            selfAthlete:addBuff(maxLevelBuff, maxLevelAthlete)
            selfAthlete:judgeEmotional(maxLevelSkill)
            selfAthlete:judgeTeamSoul(maxLevelSkill)
...
function Team:resetOnChangeToAbnormalMatchState()
    self:clearTeamManualOperate()
    self.latestPassAthlete = nil
    self.isStealOrIntercept = nil
    self.isStolenInOwnArea = false
    self.continuousCatchPassCount = 0
    ...
    for _, athlete in ipairs(self.athletes) do
        athlete:resetOnChangeToAbnormalMatchState()
    end
end

function Team:initGoalCountByGoalWay()
    for _, goalWayValue in pairs(QuestConditionConfig.GoalWayType) do
        self.goalCountByGoalWay[goalWayValue] = 0
    end
end

function Team:initSkillCount()
    for _, skill in pairs(Skills) do
        self.skillCount[skill.id] = 0
    end
end

function Team:initSkillCountByType()
    for _, skillType in pairs(AIConfig.SkillType) do
        self.skillCountByType[skillType] = 0
    end
end

function Team:initStatisticsByPosition()
    for _, position in pairs(AIConfig.positionNames) do
        self.statisticsByPosition[position] = {
            goal = 0,
            assist = 0
        }
    end
...
function Team:getSkillInstanceCount(skill)
    local count = 0
    for i, athlete in ipairs(self.athletes) do
        if athlete:hasSkill(skill) then
            count = count + 1
...
function Team:getSkillCountByBaseSkillId(baseSkillId)
    local totalCount = 0
    for skillId, count in pairs(self.skillCount) do
        if AIUtils.isSkillIdCorrespondSkill(skillId, AIUtils.getSkillById(baseSkillId)) then
...
function Team:getAbovePassDistanceCount(passDistance)
    local count = 0
    for _, passInfo in ipairs(self.passInfoList) do
        if math.cmpf(passInfo.passDistance, passDistance) >= 0 then
            count = count + 1
        end
    end
...
function Team:getBelowPassDistanceCount(passDistance)
    local count = 0
    for _, passInfo in ipairs(self.passInfoList) do
        if math.cmpf(passInfo.passDistance, passDistance) <= 0 then
            count = count + 1
...
function Team:getAboveShootDistanceCount(shootDistance)
    local count = 0
    for _, goalInfo in ipairs(self.goalInfoList) do
        if math.cmpf(goalInfo.shootDistance, shootDistance) >= 0 then
            count = count + 1
        end
    end
...
function Team:getBelowShootDistanceCount(shootDistance)
    local count = 0
    for _, goalInfo in ipairs(self.goalInfoList) do
        if math.cmpf(goalInfo.shootDistance, shootDistance) <= 0 then
            count = count + 1
...
function Team:getShootSkillGoalCount(shootSkillId)
    local count = 0
    for _, goalInfo in ipairs(self.goalInfoList) do
        if goalInfo.shootSkillId and AIUtils.isSkillIdCorrespondSkill(goalInfo.shootSkillId, AIUtils.getSkillById(shootSkillId)) then
            count = count + 1
        end
    end
...
function Team:getPassSkillGoalCount(passSkillId)
    local count = 0
    for _, goalInfo in ipairs(self.goalInfoList) do
        if goalInfo.passSkillId and AIUtils.isSkillIdCorrespondSkill(goalInfo.passSkillId, AIUtils.getSkillById(passSkillId)) then
            count = count + 1
        end
        if goalInfo.passSkillId2 and AIUtils.isSkillIdCorrespondSkill(goalInfo.passSkillId2, AIUtils.getSkillById(passSkillId)) then
            count = count + 1
        end
    end
...
function Team:getPassAndShootSkillGoalCount(passSkillId, shootSkillId)
    local count = 0
    for _, goalInfo in ipairs(self.goalInfoList) do
        if (goalInfo.passSkillId and AIUtils.isSkillIdCorrespondSkill(goalInfo.passSkillId, AIUtils.getSkillById(passSkillId))
            or goalInfo.passSkillId2 and AIUtils.isSkillIdCorrespondSkill(goalInfo.passSkillId2, AIUtils.getSkillById(passSkillId)))
            and AIUtils.isSkillIdCorrespondSkill(goalInfo.shootSkillId, AIUtils.getSkillById(shootSkillId)) then
            count = count + 1
        end
    end
...
function Team:getDribbleAndShootSkillGoalCount(dribbleSkillId, shootSkillId)
    local count = 0
    for _, goalInfo in ipairs(self.goalInfoList) do
        if goalInfo.dribbleSkillId and AIUtils.isSkillIdCorrespondSkill(goalInfo.dribbleSkillId, AIUtils.getSkillById(dribbleSkillId))
            and AIUtils.isSkillIdCorrespondSkill(goalInfo.shootSkillId, AIUtils.getSkillById(shootSkillId)) then
            count = count + 1
        end
    end
...
function Team:getGoalPositionCount(goalPositionType)
    local count = 0
    for _, goalInfo in ipairs(self.goalInfoList) do
        if goalPositionType == goalInfo.goalPositionType then
            count = count + 1
        end
    end
...
function Team:getAbovePassTimesGoalCount(passTimes)
    local count = 0
    for _, goalInfo in ipairs(self.goalInfoList) do
        if goalInfo.passTimesBeforeGoal >= passTimes then
            count = count + 1
        end
    end
...
function Team:getBelowPassTimesGoalCount(passTimes)
    local count = 0
    for _, goalInfo in ipairs(self.goalInfoList) do
        if goalInfo.passTimesBeforeGoal <= passTimes then
            count = count + 1
...
function Team:updateQuestConditionCompletionStates(questCondition)
    if questCondition.type == QuestConditionConfig.Type.SpecificPositionPlayerGoalCount
    or questCondition.type == QuestConditionConfig.Type.SpecificPositionPlayerAssistCount then
        for _, athlete in ipairs(self.athletesAll) do
            athlete.hasUncompletedQuestCondition = (AIUtils.getPosStr(athlete.role) == questCondition.position
            and not questCondition.result) or athlete.hasUncompletedQuestCondition
        end
    elseif questCondition.type == QuestConditionConfig.Type.SpecificBaseIdPlayerGoalCount
    or questCondition.type == QuestConditionConfig.Type.SpecificBaseIdPlayerAssistCount then
        for _, athlete in ipairs(self.athletesAll) do
            athlete.hasUncompletedQuestCondition = (athlete.baseId == questCondition.baseId
            and not questCondition.result) or athlete.hasUncompletedQuestCondition
        end
    elseif questCondition.type == QuestConditionConfig.Type.SkillCount
    or questCondition.type == QuestConditionConfig.Type.ShootSkillGoalCount
    or questCondition.type == QuestConditionConfig.Type.PassSkillGoalCount then
        for _, athlete in ipairs(self.athletesAll) do
            athlete.hasUncompletedQuestCondition = (athlete:hasSkill(AIUtils.getSkillById(questCondition.skillId))
            and not questCondition.result) or athlete.hasUncompletedQuestCondition
        end
    elseif questCondition.type == QuestConditionConfig.Type.PassAndShootSkillGoalCount then
        for _, athlete in ipairs(self.athletesAll) do
            athlete.hasUncompletedQuestCondition = (athlete:hasSkill(AIUtils.getSkillById(questCondition.passSkillId))
            or athlete:hasSkill(AIUtils.getSkillById(questCondition.shootSkillId)) and not questCondition.result)
            or athlete.hasUncompletedQuestCondition
        end
    end
end

function Team:resetQuestConditionCompletionStates()
    for _, athlete in ipairs(self.athletesAll) do
        athlete.hasUncompletedQuestCondition = false
    end
end
    ...
    for _, athlete in ipairs(self.athletes) do
        ...
        if skill and selector.tossCoin(skill.probability) then
            ...
            if skill:isTypeOf(Skills.DefendDeterrentEx1) then -- EX可以叠加
                ...
                if skill:isTypeOf(Skills.WallOfSanSiroEx1) and selector.tossCoin(skill.ex1SLProbability) then
                    for _, selfAthlete in ipairs(self.athletes) do
                        selfAthlete:addBuff(skill.ex1Buff, athlete)
                    end
                end
            else -- 普通不能叠加
                if skill.level > maxLevel then
...
function Team:judgeTheIcasillas(shooter)
    for _, athlete in ipairs(self.athletes) do
        athlete:judgeTheIcasillas(shooter)
    end
end

function Team:judgeLRSgerrardSecond()
    for _, athlete in ipairs(self.athletes) do
        athlete:judgeLRSgerrardSecond()
    end
end
