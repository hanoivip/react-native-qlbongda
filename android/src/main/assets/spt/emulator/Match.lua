...
function Match:init(initializer)
    ...
    local questConditions = initializer.baseInfo.questConditions
    if questConditions then
        self:initQuestConditions(questConditions)
    end
    ...
    self.needJudgeTeamSkillsOnNextCatch = true
    self:changeState("PrepareToKickOff")
end

function Match:initQuestConditions(questConditions)
    self.questConditions = questConditions

    self.hasAthleteQuestConditions = false
    for _, questCondition in ipairs(questConditions) do
        questCondition.checkFunc = QuestConditionConfig.CheckFunc[questCondition.type]
        self.playerTeam:updateQuestConditionCompletionStates(questCondition)
        if questCondition.type == QuestConditionConfig.Type.SkillCount
                or questCondition.type == QuestConditionConfig.Type.ShootSkillGoalCount
                or questCondition.type == QuestConditionConfig.Type.PassSkillGoalCount
                or questCondition.type == QuestConditionConfig.Type.SpecificPositionPlayerGoalCount
                or questCondition.type == QuestConditionConfig.Type.SpecificPositionPlayerAssistCount
                or questCondition.type == QuestConditionConfig.Type.SpecificBaseIdPlayerGoalCount
                or questCondition.type == QuestConditionConfig.Type.SpecificBaseIdPlayerAssistCount
                then
            self.hasAthleteQuestConditions = true
        end
    end
end
...
function Match:switchRole(isGoal)
    ...
    self.needJudgeTeamSkillsOnNextCatch = true
    self.lastSwitchRoleIsGoal = true
end
...
function Match:updateQuestConditions()
    if self.questConditions then
        self.questConditionStateChanged = false

        local shouldGameOver
        for i, questCondition in ipairs(self.questConditions) do
            local tmpResult = questCondition.result
            local tmpCount = questCondition.count

            questCondition.result, questCondition.count, conditionShouldGameOver = questCondition.checkFunc(self, questCondition)
            shouldGameOver = shouldGameOver or conditionShouldGameOver

            if questCondition.result ~= tmpResult or questCondition.count ~= tmpCount then
                self.questConditionStateChanged = true
            end
        end

        if self.questConditionStateChanged then
            self.playerTeam:resetQuestConditionCompletionStates()
...
function Match:judgeAfterSwitchRoleSkills()
    ...
    if not self.lastSwitchRoleIsGoal then
        self.lastSwitchRoleIsGoa = nil
...
function Match:judgeAfterGoalSkills()
    ...
    -- 安菲尔德鼓舞 进球丢球都鼓励
    self.attackTeam:judgeLRSgerrardSecond()
    self.defenseTeam:judgeLRSgerrardSecond()
...
function Match:changeState(stateName)
    if stateName ~= "NormalPlayOn" then
        self.ball.nextTask = nil
        self.isGoal = false
        self.playerTeam:resetOnChangeToAbnormalMatchState()
        self.opponentTeam:resetOnChangeToAbnormalMatchState()
    end
    ...
    if stateName == "GameOver" and self.questConditions then
        for _, condition in ipairs(self.questConditions) do
            condition.checkFunc = nil
        end
    end
end
