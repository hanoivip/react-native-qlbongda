...
function Athlete:ctor()
    ...
    -- 球员的初始额外全属性加成，比如环球挑战赛中租借球员全属性加成
    self.initBuffBonus = nil
    self.lastTouchBallRole = nil
    self.lastDribbleSkillId = nil
end
...
function Athlete:selectCatchAnimation()
    ...
    if isIntercept then
        ...
        if ballNextTask.isAccurateGuidePass then
            local skill = ballNextTask.passer:getSkill(Skills.AccurateGuide)
            if skill then
                self:addBuff(skill.buff, ballNextTask.passer)
            end
        end
        if ballNextTask.isCornerkick then
            self.isCatchCornerKick = true
        end
...
function Athlete:catch()
    ...
    if self.match.needJudgeTeamSkillsOnNextCatch then
        self.match.needJudgeTeamSkillsOnNextCatch = nil
        self.match:judgeAfterSwitchRoleSkills()
    end
    self:judgeLRConfuseMaster()
end
...
function Athlete:intercept()
    ...
    self:judgeLRAnestaSecond()
    self:judgeLRDeterrentForce()
...
function Athlete:startSteal()
    ...
    if stealTargetAthlete ~= nil then
        ...
        stealTargetAthlete:judgeLRButcherInMidfield(self)
    end
...
function Athlete:goal()
    if not self.match.isInPenaltyShootOut then
        ...
        self.team.statisticsByPosition[AIUtils.getPosStr(self.role)].goal
            = self.team.statisticsByPosition[AIUtils.getPosStr(self.role)].goal + 1

        if self.baseId then
            self.team.statisticsByBaseId[self.baseId].goal = self.team.statisticsByBaseId[self.baseId].goal + 1
        end

        self:updateGoalCountByGoalWay()

        table.insert(self.team.goalInfoList, {
            shootDistance = vector2.dist(self.lastShootAction.startPosition, self.enemyTeam.goal.center),
            passTimesBeforeGoal = self.team.passTimesBeforeGoal,
            shootSkillId = self.lastShootAction.shootSkillId,
            passSkillId = self.lastShootAction.passSkillId,
            passSkillId2 = self.lastShootAction.passSkillId2,
            dribbleSkillId = self.lastShootAction.dribbleSkillId,
            goalPositionType = self:getGoalPositionType(),
        })
    end
    ...
    local event = {
        ...
        questConditions = self.team.match.questConditions,
    }
    local ball = self.match.ball
    if ball.preTouchAthlete ~= nil and ball.preTouchAthlete ~= self and ball.preTouchAthlete.team == self.team then
        ...
        event.assisterPos = ball.preTouchAthlete.lastTouchBallRole
        self.team.statisticsByPosition[AIUtils.getPosStr(ball.preTouchAthlete.lastTouchBallRole)].assist
            = self.team.statisticsByPosition[AIUtils.getPosStr(ball.preTouchAthlete.lastTouchBallRole)].assist + 1

        -- self.team.statisticsByBaseId[ball.preTouchAthlete.baseId].assist
        --     = self.team.statisticsByBaseId[ball.preTouchAthlete.baseId].assist + 1
    end
...
function Athlete:getGoalPositionType()
    local isInTopGoalArea = math.cmpf(self.lastShootAction.targetPositionHeight, Field.goalHeight / 2) >= 0
    local targetXSign = math.sign(self.lastShootAction.targetPosition.x) == 0 and 1 or math.sign(self.lastShootAction.targetPosition.x)
    local goalPositionType
    if self.team:getSign() * targetXSign > 0 then
        goalPositionType = isInTopGoalArea and QuestConditionConfig.GoalPositionType.LeftTop or QuestConditionConfig.GoalPositionType.LeftBottom
    else
        goalPositionType = isInTopGoalArea and QuestConditionConfig.GoalPositionType.RightTop or QuestConditionConfig.GoalPositionType.RightBottom
    end

    return goalPositionType
end

function Athlete:updateGoalCountByGoalWay()
    if self.shootType == AIUtils.shootType.header then
        self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.header]
        = self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.header] + 1
    elseif self.shootType == AIUtils.shootType.volleyShoot then
        self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.volleyShoot]
        = self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.volleyShoot] + 1
    elseif self.shootType == AIUtils.shootType.freeKick then
        self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.centerDirectFreeKick]
        = self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.centerDirectFreeKick] + 1
    elseif self.shootType == AIUtils.shootType.penaltyKick then
        self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.penaltyKick]
        = self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.penaltyKick] + 1
    end

    if self.isCatchCornerKickShoot then
        self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.catchCornerKick]
        = self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.catchCornerKick] + 1

        self.isCatchCornerKickShoot = nil
    end

    if self.isCatchWingDirectFreeKickShoot then
        self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.catchWingDirectFreeKick]
        = self.team.goalCountByGoalWay[QuestConditionConfig.GoalWayType.catchWingDirectFreeKick] + 1

        self.isCatchWingDirectFreeKickShoot = nil
    end
end

function Athlete:isTargetedByQuestCondition(questCondition)
    if questCondition.type == QuestConditionConfig.Type.SpecificBaseIdPlayerGoalCount
            or questCondition.type == QuestConditionConfig.Type.SpecificBaseIdPlayerAssistCount then
        return self.baseId == questCondition.baseId
    elseif questCondition.type == QuestConditionConfig.Type.SpecificPositionPlayerGoalCount
            or questCondition.type == QuestConditionConfig.Type.SpecificPositionPlayerAssistCount then
        return AIUtils.getPosStr(self.role) == questCondition.position
    elseif questCondition.type == QuestConditionConfig.Type.SkillCount
        ...
        if not questCondition.skillId then
            return false
        end
        for i, skillInstance in ipairs(self.skills) do
            if AIUtils.isSkillIdCorrespondSkill(questCondition.skillId, skillInstance.class) then
                return true
            end
        end
        return false
...
function Athlete:isTargetedByQuestConditions()
    if self.match.questConditions == nil then
        return false
    end
    for i, questCondition in ipairs(self.match.questConditions) do
        if self:isTargetedByQuestCondition(questCondition) then
            return true
        end
    end
    return false
...
function Athlete:resetOnChangeToAbnormalMatchState()
    if self.behaviorTree ~= nil then
        self:stopAnimation(true)
        self:logDebug("stop athlete %d, change match state", self.id)
    end
    ...
    self:setMoveStatus(0)
    self:resetMoveStatusRemainingTime()
    self.graspBall = nil
    self.isInterceptOrStealBallNoCounterAttack = nil
    self.breakThroughDefendInfo = nil
    self.shouldHeadingDual = nil
    self.lastDribbleSkillId = nil
    self:setMark(nil)
    self:setCover(nil)
end
