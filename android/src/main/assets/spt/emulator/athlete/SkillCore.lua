    ...
    if showCaster then
        ...
        for i, skillInstance in ipairs(self.skills) do
            if skillInstance.class == skill then
                ...
                self.team.skillCount[skillInstance.id] = (self.team.skillCount[skillInstance.id] or 0) + 1
                for _, skillType in pairs(AIConfig.SkillType) do
                    if AIUtils.isSkillTypeOfOneOfSkills(skillInstance.class, AIConfig.SkillsByType[skillType]) then
                        self.team.skillCountByType[skillType] = self.team.skillCountByType[skillType] + 1
                    end
                end
                return skillInstance
            end
...
function Athlete:castPassSkills()
    for i, buff in ipairs(self.buffs) do
        if buff.type.skill then
            ...
            or (skillInstance:isTypeOf(Skills.ThroughBall) and buff.type.remark == "selfBuff")
...
function Athlete:getAbilities()
    if self.cachedAbilities == nil or self.cachedAbilities.isDirty then
        ...
        if self.team.roleInitBuffBonus ~= nil then
            local buffBonus = self.team.roleInitBuffBonus[tostring(self.role)]
            if buffBonus ~= nil then
                makeAbilitiesAddRatioEffect(self.cachedAbilities, buffBonus, self.initAbilities)
            end
        end
    end
...
function Athlete:judgeSlidingTackleEx1(targetEnemy)
    ...
    if not self:isDivingEx1Blocked() and skill and selector.tossCoin(skill.ex1Probability) then
        if skill:isTypeOf(Skills.BlueAndWhiteBanditsEx1) then
            if targetEnemy:hasBuff(Skills.SlidingTackleEx1, "mark") then
                for _, buffInstance in ipairs(targetEnemy.buffs) do
                    if buffInstance.type.skill:isTypeOf(Skills.SlidingTackleEx1) and buffInstance.type.remark == "mark" then
                        buffInstance.remainingTime = skill.durationConfig
                    end
                end
            else
                local unmarkedNonFStartSkills = targetEnemy:getUnmarkedNonFStartSkills()
                local markedSkill = selector.randomSelect(unmarkedNonFStartSkills)
                if markedSkill then
                    markedSkill.isDisabled = true
                    targetEnemy:removeBuffs(markedSkill.class)
                    skill.ex1Debuff.markedSkillId = markedSkill.id
                end
            end
        end
...
function Athlete:judgePerpetualMotionMachineEx1()
    ...
    if skill then
        ...
        if skill:isTypeOf(Skills.Tireless) or skill:isTypeOf(Skills.CzechIronMan) then
            local sprSkill = self:getCooldownSkill(Skills.SpiritualEncouragement)
            if sprSkill and selector.tossCoin(sprSkill.probability) then
                self:castSkill(sprSkill.class)
                for _, friend in ipairs(self.team.athletes) do
                    friend:addBuff(sprSkill.buff, self)
                end
            end
        end
...
function Athlete:judgeOrganizeWallEx1()
    ...
    if skill ~= nil then
        if self.match.frozenType == "WingDirectFreeKick" then
            ...
            self:addBuff(skill.buff, enemyGk)
            self:castSkill(skill.class)
...
function Athlete:judgeTheIcasillas(shooter)
    local skill = self:getCooldownSkill(Skills.TheIcasillas)
    if skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        shooter:addBuff(skill.debuff, self)
    end
end

function Athlete:judgeSmartFoxInTheBox()
    if Field.isInPenaltyArea(self.position, self.team:getSign()) then
        local skill = self:getCooldownSkill(Skills.SmartFoxInTheBox)
        if skill and selector.tossCoin(skill.probability) then
            self:castSkill(skill.class)
            self:addBuff(skill.buff, self)
        end
...
function Athlete:judgeAresShotBlasting()
    local skill = self:getCooldownSkill(Skills.AresShotBlasting)
    if skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        local enemyGK = self.enemyTeam.athleteOfRole[26]
        for _, buff in ipairs(enemyGK.buffs) do
            if buff.type.skill:isTypeOf(Skills.AresShotBlasting) then
                buff.remainingTime = skill.durationConfig
            end
        end
        enemyGK:addBuff(skill.debuff, self)
    end
end

function Athlete:judgeAccurateGuide()
    local skill = self:getCooldownSkill(Skills.AccurateGuide)
    if skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        self:addBuff(skill.selfBuff, self)
    end
...
function Athlete:judgeSpiritualEncouragement(skill) 
    if skill:isTypeOf(Skills.Tireless) or skill:isTypeOf(Skills.CzechIronMan) then
        local sprSkill = self:getCooldownSkill(Skills.SpiritualEncouragement)
        if sprSkill and selector.tossCoin(sprSkill.probability) then
            self:castSkill(sprSkill.class)
...
function Athlete:judgeLRWingWizard(withBallSkillId)
    if withBallSkillId and string.match(withBallSkillId, "B01") then
        local skill = self:getCooldownSkill(Skills.LRWingWizard)
        if skill and selector.tossCoin(skill.probability) then
            self:castSkill(skill.class)
            for _, enemy in ipairs(self.enemyTeam.athletes) do
                for _, debuff in ipairs(skill.debuffs) do
                    enemy:addBuff(debuff, self)
                end
            end
        end
    end
...
function Athlete:judgeLRGoalKeeper()
    local abilitySum = 0
    local skill = self:getCooldownSkill(Skills.LRGoalKeeper)
    if skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        for _, friend in ipairs(self.team.athletes) do
            if friend:isBack() then
                abilitySum = abilitySum + (friend:getAbilities().intercept + friend:getAbilities().steal) * skill.addRatio
            end
        end
    end
    return abilitySum
...
function Athlete:castLRPassIsDribble()
    local skill = self:getCooldownSkill(Skills.LRPassIsDribble)
    if skill then
        self:castSkill(skill.class)
    end
end

function Athlete:judgeLRPassIsDribble()
    local abilitySum = 0
    local skill = self:getCooldownSkill(Skills.LRPassIsDribble)
    if skill and selector.tossCoin(skill.probability) then
        abilitySum = abilitySum + self:getAbilities().pass * skill.addRatio
    end
    return abilitySum
end

function Athlete:upgradeLROverwhelmingAttack()
    local skill = self:getSkill(Skills.LROverwhelmingAttack)
    if skill then
        self:castSkill(skill.class)
        skill.addRatio = skill.addRatio + skill.upgradeRatio
    end
end

function Athlete:judgeLROverwhelmingAttack()
    local skill = self:getCooldownSkill(Skills.LROverwhelmingAttack)
    local abilitySum = 0
    if skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        abilitySum = abilitySum + self:getAbilitiesSum() * skill.addRatio
    end
    return abilitySum
end

function Athlete:judgeRedArmyHeavyGunnerEx1(distanceFactor)
    if self:hasBuff(Skills.HeavyGunner, "baseBuff") then
        local skill = self:getCooldownSkill(Skills.RedArmyHeavyGunnerEx1)
        if skill and selector.tossCoin(skill.ex1SLProbability) then
            self:addBuff(skill.ex1SLBuff, self)
            return skill.ex1DistanceRatio
        end
    end
    return 1
end

function Athlete:judgeLRConfuseMaster()
    local skill = self:getCooldownSkill(Skills.LRConfuseMaster)
    if skill and selector.tossCoin(skill.probability) then
        local targetEnemy
        local candidateEnemies = {}
        for _, enemy in ipairs(self.enemyTeam.athletes) do
            if enemy:isBack() then
                table.insert(candidateEnemies, enemy)
            end
        end

        if #candidateEnemies > 0 then
            local targetEnemy = selector.randomSelect(candidateEnemies)
            if targetEnemy then
                local enemySkills = targetEnemy:getUnmarkedNonFStartSkills()        
                local targetSkill = selector.randomSelect(enemySkills)
                if targetSkill then
                    targetSkill.isDisabled = true
                    skill.debuff.markedSkillId = targetSkill.id
                    targetEnemy:removeBuffs(targetSkill.class)
                end
                targetEnemy:addBuff(skill.debuff, self)
                self:castSkill(skill.class)
            end
        end
    end
...
function Athlete:judgeLRPharaoh()
    local skill = self:getCooldownSkill(Skills.LRPharaoh)
    if skill and selector.tossCoin(skill.probability) then
        self:addBuff(skill.buff, self)
        self:castSkill(skill.class)
    end
...
function Athlete:judgeLRPharaohAfterGoal()
    local skill = self:getSkill(Skills.LRPharaoh)
    if skill then
        for _, enemy in ipairs(self.enemyTeam.athletes) do
           enemy:addBuff(skill.debuff, self) 
        end
...
function Athlete:judgeLRDeterrentForce()
    local skill = self:getSkill(Skills.LRDeterrentForce)
    if skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        for _, enemy in ipairs(self.enemyTeam.athletes) do
           enemy:addBuff(skill.debuff, self) 
...
function Athlete:judgeLRButcherInMidfield(targetEnemy)
    local skill = self:getCooldownSkill(Skills.LRButcherInMidfield)
    if skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        targetEnemy:addBuff(skill.debuff1, self)
        for _, buffInstance in ipairs(targetEnemy.buffs) do
            if buffInstance.type.skill:isTypeOf(Skills.LRButcherInMidfield) and buffInstance.type.remark and buffInstance.type.remark == "duration" then
                buffInstance.remainingTime = skill.debuff2.duration
            end
        end
        targetEnemy:addBuff(skill.debuff2, self)
    end
end

function Athlete:judgeLRSgerrardSecond()
    local skill = self:getCooldownSkill(Skills.LRSgerrardSecond)
    if skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        for _, friend in ipairs(self.team.athletes) do
            friend:addBuff(skill.buff, self)
...
function Athlete:judgeLRThenrySecond()
    local skill = self:getCooldownSkill(Skills.LRThenrySecond)
    if skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        self:addBuff(skill.buff, self)
    end
...
function Athlete:judgeLRKakaSecond()
    local skill = self:getCooldownSkill(Skills.LRKakaSecond)
    if not self:hasBuff(Skills.LRKakaSecond, "blockInfluence") and skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        self:addBuff(skill.buff, self)
    end
end

function Athlete:judgeLRKakaSecondEffect()
    if not Field.isInPenaltyArea(self.position, self.team:getSign()) and self:hasBuff(Skills.LRKakaSecond, "blockInfluence") then
        skill = self:getCooldownSkill(Skills.LRKakaSecond)
        self:castSkill(skill.class)
        return true
    end
end

function Athlete:judgeLRAnestaSecond()
    local skill = self:getCooldownSkill(Skills.LRAnestaSecond)
    if skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        for _, enemy in ipairs(self.enemyTeam.athletes) do
            for _, buffInstance in ipairs(enemy.buffs) do
                if buffInstance.type.skill:isTypeOf(Skills.LRAnestaSecond) and buffInstance.type.remark and buffInstance.type.remark == "resetTime" then
                    buffInstance.remainingTime = skill.debuff.duration
                end
            end
            enemy:addBuff(skill.debuff, self)
        end
...
function Athlete:judgeLRDsimeoneSecond()
    local skill = self:getCooldownSkill(Skills.LRDsimeoneSecond)
    if skill and selector.tossCoin(skill.probability) then
        self:castSkill(skill.class)
        for _, friend in ipairs(self.team.athletes) do
            friend:addBuff(skill.extraBuff, self)
        end
    end
end