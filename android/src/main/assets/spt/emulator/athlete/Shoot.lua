...
function Athlete:preShoot()
    ...
    if self.outputActionStatus == nil then
        ...
        self.shootType = self:getShootType()

        if self.isCatchCornerKick then
            self.isCatchCornerKickShoot = true
            self.isCatchCornerKick = nil
        end
    end
...
function Athlete:evaluateShoot(targetPosition, targetPositionHeight, controlPoint, flyDuration, isShootHigh, actionType, enableAutoShootWide)
    ...
    self.enemyTeam:judgeTheIcasillas(self)
    self:judgeSmartFoxInTheBox()
    self:judgeAresShotBlasting()
    self:judgeLRPharaoh()
    self:judgeLRThenrySecond()
    ...
    shootAction.isGoal = true
    shootAction.shootResult = AIUtils.shootResult.goal
    self:upgradeLROverwhelmingAttack()
...
function Athlete:getShootType()
    -- 解决进球方式优先级问题
    local preferType
    if self.questConditions then
        for _, condition in ipairs(self.questConditions) do
            if condition.type == QuestConditionConfig.Type.GoalWayCount then
                preferType = condition.goalWayType
            end
        end
    end

    local shootType = AIUtils.shootType.normalGroundShoot
    if self.catchType == AIUtils.catchType.VolleyShoot
        or self.catchType == AIUtils.catchType.NormalVolleyShoot then
        shootType = AIUtils.shootType.volleyShoot
        if preferType and shootType == preferType then
            return shootType
        end
    end

    if self.catchType == AIUtils.catchType.PowerfulHeader
        or self.catchType == AIUtils.catchType.NormalHeader then
        shootType = AIUtils.shootType.header
        if shootType == preferType then
            return shootType
        end
    end

    if self.isHeavyGunner
        and not Field.isInPenaltyArea(self.position, self.team:getSign()) then
        shootType = AIUtils.shootType.longShoot
        if preferType and shootType == preferType then
            return shootType
        end
    end

    if self.match.lastMatchState == "PenaltyKick"
        or self.match.lastMatchState == "PenaltyShootOutKick" then
        shootType = AIUtils.shootType.penaltyKick
        if preferType and shootType == preferType then
            return shootType
        end
    elseif self.match.lastMatchState == "CenterDirectFreeKick" then
        shootType = AIUtils.shootType.freeKick
        if preferType and shootType == preferType then
            return shootType
        end
    end
...
function Athlete:shoot()
    ...
    shootAction.shootSkillId = self.shootSkillId
    shootAction.passSkillId = ball.nextTask and ball.nextTask.skillId
    shootAction.passSkillId2 = ball.nextTask and ball.nextTask.skillId2
    ...
    -- 点球杀手ex1，扑点球成功时生效
    if goalKeeper:hasBuff(Skills.PenaltyKickKillerEx1, "buffSign")
      and (ball.nextTask.shootResult == AIUtils.shootResult.catch or ball.nextTask.shootResult == AIUtils.shootResult.saveBounce) then
        goalKeeper:judgePenaltyKickKillerEx1()
    end
    ...
    shootAction.dribbleSkillId = self.lastDribbleSkillId
