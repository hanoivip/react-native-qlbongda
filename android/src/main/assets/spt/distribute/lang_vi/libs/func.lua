...
function string.formatIntWithTenThousands(amount)
    ...
    if amount >= 1000000000 then
        local part = math.floor(amount / 1000000000)
        strResult = lang.transstr("logogramMoneyBillon", part)
        amount = amount % 1000000000
        amount = math.floor(amount / 1000)
        if amount > 0 then
            strResult = lang.transstr("logogramMoneyMillon", strResult .. amount)
        end
    elseif amount >= 1000000 then
        local part = math.floor(amount / 1000000)
        strResult = lang.transstr("logogramMoneyMillon", part)
        amount = amount % 1000000
        amount = math.floor(amount / 1000)
        if amount > 0 then
            strResult = lang.transstr("logogramMoneyThousand", strResult .. amount)
        end
    elseif amount >= 1000 then
        amount = math.floor(amount / 10) / 100
        strResult = lang.transstr("logogramMoneyThousand", amount)
    else
...
function string.formatNumWithUnit(num, n)
    ...
    elseif num < 1000000 then
        num = math.floor(num / math.pow(10, 3 - n)) / math.pow(10, n)
        return lang.transstr("logogramMoneyThousand", num) 
    elseif num < 1000000000 then
        num = math.floor(num / math.pow(10, 6 - n)) / math.pow(10, n)
        return lang.transstr("logogramMoneyMillon", num)
    else
        num = math.floor(num / math.pow(10, 9 - n)) / math.pow(10, n)
        return lang.transstr("logogramMoneyBillon", num)
    end
end
