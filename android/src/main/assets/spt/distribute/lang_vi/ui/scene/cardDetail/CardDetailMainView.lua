...
function CardDetailMainView:InitView(cardDetailModel, isOperable, currentPage)
    ...
    self:SetFeatureLayout(cardModel)
...
function CardDetailMainView:SetFeatureLayout(cardModel)
    local isFeatureOpen = cardModel:HasCoachFeature()
    ...
    GameObjectHelper.FastSetActive(self.btnFeatureInfo.gameObject, isFeatureOpen)
end
