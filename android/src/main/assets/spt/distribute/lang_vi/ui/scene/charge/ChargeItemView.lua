...
local currencySymbol = " VND"
local function formatCurrency(amount)
    if amount > 1000 then
        return string.formatIntWithTenThousands(amount) .. currencySymbol
    else
        return tostring(amount) .. currencySymbol
    end
end
...
function ChargeItemView:Init(model)
    ...
    local price = model:GetShowPrice()
    ...
    self.price.text = formatCurrency(price)
