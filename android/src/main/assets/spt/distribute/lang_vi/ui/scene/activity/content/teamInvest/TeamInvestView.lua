...
function TeamInvestView:ctor()
    ...
    self.newYear = self.___ex.newYear
...
function TeamInvestView:RefreshContent()
    ...
    self:InitBackgroungImage()
end

function TeamInvestView:InitBackgroungImage()
    local imgPath = "Assets/CapstonesRes/Game/UI/Scene/Activties/TeamInvest/Image/BG_%s.png"
    local boardType = self.activityModel:GetBoardType()
    self.bgImg.overrideSprite = res.LoadRes(string.format(imgPath, boardType))

    ChristmasBoardType = 2
    GameObjectHelper.FastSetActive(self.newYear, tostring(boardType) == tostring(ChristmasBoardType))
end
