...
function LoginCtrl.OnLoginSuccess(channel)
    ...
    if not luaevt.trig("___EVENT__NOT_OPEN_FORBIDDEN_VN") then
...
function LoginCtrl.OnLoginFail(channel)
    LoginCtrl.instance.view:OnLoginErr()
end
...
local function JudgeEnterScene(needChangeToTeamLogoCreate)
    clr.coroutine(function()
        ...
        if api.success(response) then
            ...
            local playerName = data.info.name == "" and "none" or data.info.name
            local diamond = playerInfoModel:GetDiamond()
            luaevt.trig("SDK_Report", "home_enter", data.info._id, playerName, data.info.lvl, server.id, server.name, diamond)
...
function LoginCtrl:Refresh()
    ...
    self.view:coroutine(function()
        ...
        local marblesExchangeItem = self:GetStaticTableMaxId(MarblesExchangeItem)
