...
local DescCharacter = { ["\\x"] = 1, ["\\y"] = 2, ["\\a"] = 3, ["\b"] = 4, ["\\c"] = 5 }
local function GetSkillExtraDesc(descArray, attributePlusTable)
    local skillDesc = ""
    local k = 1
    for i, desc in ipairs(descArray) do
        local tDesc = desc
        for index, v in pairs(DescCharacter) do
            if attributePlusTable[v] then
                local r, c = string.gsub(tDesc, index, attributePlusTable[v])
                if r then
                    tDesc = r
                end
            end
