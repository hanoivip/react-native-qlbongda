...
function RewardDataCtrl:BuildReward()
    ...
    self:BuildFancyCardPieceReward()
end
...
--构建梦幻卡碎片奖励
function RewardDataCtrl:BuildFancyCardPieceReward()
    if type(self.rewardData.fancyCardPiece) == "table" then
        local cardPieceListData = self.rewardData.fancyCardPiece
        if self.isReceive then
            for i, cardPieceData in ipairs(cardPieceListData) do
                local fancyCardPieceModel = FancyCardPieceModel.new()
                fancyCardPieceModel:InitWithStatic(cardPieceData.id, cardPieceData.add)
                self:InstantiateFancyCardPieceBox(fancyCardPieceModel, MailRewardType.FancyCardPiece)
            end
        else
            for i, cardPieceData in ipairs(cardPieceListData) do
                local fancyCardPieceModel = FancyCardPieceModel.new()
                fancyCardPieceModel:InitWithStatic(cardPieceData.id, cardPieceData.num)
                self:InstantiateFancyCardPieceBox(fancyCardPieceModel, MailRewardType.FancyCardPiece)
            end
        end
    end
...
-- 实例化梦幻卡碎片
function RewardDataCtrl:InstantiateFancyCardPieceBox(fancyCardPieceModel, rType)
    local avatarBoxObj, avatarBoxView = res.Instantiate("Assets/CapstonesRes/Game/UI/Common/Part/FancyCardPiece.prefab")
    avatarBoxObj.transform:SetParent(self.parentTrans, false)
    avatarBoxView:InitView(fancyCardPieceModel, self.isShowName, self.isShowCount, self.isShowDetail)
    if self.itemParams ~= nil then
        if self.itemParams.nameColor ~= nil and self.itemParams.nameShadowColor ~= nil then
            avatarBoxView:SetNameColor(self.itemParams.nameColor, self.itemParams.nameShadowColor)
        end
    ...
    self:SaveItemModel(rType, fancyCardPieceModel)
end
