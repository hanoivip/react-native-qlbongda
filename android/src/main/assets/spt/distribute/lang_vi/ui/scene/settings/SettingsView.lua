...
function SettingsView:ctor()
    ...
    -- 套装按钮，拜仁&皇马&巴萨
    self.suitBtns = self.___ex.suitBtns
end
...
function SettingsView:start()
    ...
    -- 拜仁&皇马&巴萨
    for suitKey, luaBtn in pairs(self.suitBtns) do
        luaBtn:regOnButtonClick(function()
            self:OnBtnSuitBtnClick(suitKey)
        end)
    end
...
function SettingsView:SwitchPage(index)
    ...
    if index == 0 then
        ...
        self.scrollRect.anchoredPosition = Vector2(-527, self.scrollRect.anchoredPosition.y)
    end
end
...
function SettingsView:InitSpecificTeam(data)
    if next(data.logoShirt.specificTeam) then
        for suitKey, v in pairs(data.logoShirt.specificTeam) do
            if self.suitBtns[suitKey] ~= nil then
                GameObjectHelper.FastSetActive(self.suitBtns[suitKey].gameObject, true)
            end
        end
    else
        GameObjectHelper.FastSetActive(self.leftPageBtn.gameObject, false)
...
function SettingsView:OnBtnSuitBtnClick(suitKey)
    if self.specialTeam ~= nil and type(self.specialTeam) == "function" then
        self.specialTeam(suitKey)
    end
end
