...
function PlayerInfoModel:SetDiamond(diamond)
    ...
    local reportType
    if consumeDiamondNumber >= 0 then
        reportType = "add_currency"
    else
        reportType = "cost_currency"
    end
    local aftLvl = self:GetLevel()
    ...
    local num = tostring(math.abs(consumeDiamondNumber))
    luaevt.trig("SDK_Report", reportType, roleId, aftLvl, roleName, serverCode, serverName, "Diamond", num)
    self:OnPlayerInfoChanged()
end
