...
function MedalRuleView:InitLocalTable()
    local allNames = { "C", "B", "A", "S", "SS" }
    local alldes1 = { "+10", "+20", "+30", "+40", "+50" }
    local alldes2 = { "nil", "0.1% - 1%", "1% - 2%", "2% - 4%", "4% - 8%" }
    for k,v in pairs(self.allItem) do
        k = tonumber(k)
        local itemData = {
            mName = lang.transstr("medal_quality_name") .. allNames[k],
            des1 = lang.transstr("medal_quality_add_property") .. alldes1[k],
            des2 = (k > 1 ) and alldes2[k] or lang.trans("none"),
            des3 = (k > 2 ) and lang.trans("medal_quality_add_level",k - 2) or lang.trans("none")
        }
        v:InitView(itemData)
    end
end
