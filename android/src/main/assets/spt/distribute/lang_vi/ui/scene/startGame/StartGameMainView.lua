...
function StartGameMainView:IsShowArenaRedPoint()
    ...
    if tonumber(arenaZone) > 0 or tonumber(arenaHonor) > 0 then
        isShowRedPoint = true
    end
...
function StartGameMainView:IsShowCompeteEmailPoint()
    local email = ReqEventModel.GetInfo("worldTournamentEmail")
    GameObjectHelper.FastSetActive(self.competeRedPoint, tonumber(email) > 0)
end
...
function StartGameMainView:EnterScene()
    ...
    EventSystem.AddEvent("ReqEventModel_worldTournamentEmail", self, self.IsShowCompeteEmailPoint)
end
...
function StartGameMainView:ExitScene()
    ...
    EventSystem.RemoveEvent("ReqEventModel_worldTournamentEmail", self, self.IsShowCompeteEmailPoint)
end
