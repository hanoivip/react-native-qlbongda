...
function LoginView:start()
    self:InitAppInfo()
    GameObjectHelper.FastSetActive(self.logoutButton.gameObject, false)
    if luaevt.trig("___EVENT__NOT_OPEN_FORBIDDEN_VN") then
        GameObjectHelper.FastSetActive(self.noticeButton.gameObject, false)
    end
end

function LoginView:OnLoginComplete()
    GameObjectHelper.FastSetActive(self.mask.gameObject, false)
end

function LoginView:OnLoginErr()
    GameObjectHelper.FastSetActive(self.mask.gameObject, false)
end
...
function LoginView:InitAppInfo()
    ...
    GameObjectHelper.FastSetActive(self.mask.gameObject, true)
end
