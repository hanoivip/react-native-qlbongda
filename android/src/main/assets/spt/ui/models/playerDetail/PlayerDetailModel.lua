...
function PlayerDetailModel:InitWithProtocol(playerDetailData)
    if playerDetailData then
        ...
        -- 屏蔽发言相关
        self.cacheData.isBlock = playerDetailData.isBlock
        self.cacheData.isUseBlock = playerDetailData.isUseBlock
        if playerDetailData.blockRemainCnt then
            self.cacheData.blockRemainCnt = playerDetailData.blockRemainCnt
        end
    end
end
...
-- 被屏蔽发言的玩家
function PlayerDetailModel:SetIsBlock(isBlock)
    self.cacheData.isBlock = isBlock
end

function PlayerDetailModel:GetIsBlock()
    return self.cacheData.isBlock
end

-- 是否被该玩家屏蔽
function PlayerDetailModel:GetIsUseBlock()
    return self.cacheData.isUseBlock
end

-- 获取剩余可屏蔽玩家数
function PlayerDetailModel:GetBlockRemainCnt()
    return tonumber(self.cacheData.blockRemainCnt)
end
