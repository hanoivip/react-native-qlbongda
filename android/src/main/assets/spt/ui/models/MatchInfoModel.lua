...
function MatchInfoModel:initQuestConditions()
    self.questConditions = self.data.baseInfo.questConditions
end

function MatchInfoModel:hasQuestConditions()
    return self.questConditions ~= nil
end

function MatchInfoModel:getQuestConditions()
    return self.questConditions
end

function MatchInfoModel:updateGoalCountQuestConditionsStatus(playerScore, opponentScore)
    if self.questConditions ~= nil then
        for _, condition in ipairs(self.questConditions) do
            if condition.type == 1 then
                condition.count = playerScore
                condition.result = condition.count >= condition.targetCount
            end
            if condition.type == 2 then
                condition.count = playerScore - opponentScore
                condition.result = condition.count >= condition.targetCount
            end
            if condition.type == 3 then
                condition.count = playerScore - opponentScore
                condition.result = condition.count >= condition.targetCount
            end
            if condition.type == 6 then
                condition.count = opponentScore
                condition.result = condition.count <= condition.targetCount
            end
        end
        EventSystem.SendEvent("Match_UpdateQuestConditions", self.questConditions)
    end
end

function MatchInfoModel:getConditionsResult()
    local ret = {}
    for _, condition in ipairs(self.questConditions) do
        table.insert(ret, condition.result)
    end
    return ret
end

function MatchInfoModel:updateConditionsResultByEvent(playerEvent, opponentEvent)
    if self.questConditions ~= nil then
        local can = {}
        table.imerge(can, playerEvent)
        table.imerge(can, opponentEvent)
        table.sort(can, function(a, b) return a.time > b.time end)

        if #can > 0 then
            self.questConditions = can[1].questConditions
        end
        EventSystem.SendEvent("Match_UpdateQuestConditions", self.questConditions)
    end
end
