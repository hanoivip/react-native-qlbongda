...
function BaseCardModel:IsNotAllowSell()
    local lockTypes = self.lockHelper:IsLockByLockTypes(self.cacheData.lock, LockTypeFilter.CanNotSell)
     return lockTypes
end
...
function BaseCardModel:GetMedalMaxPos()
    return CardQuality[tostring(self:GetCardQuality())].availableMedal
end
