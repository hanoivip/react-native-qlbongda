...
function RewardBoxView:BuildView()
    ...
    -- 梦幻卡碎片 道具
    elseif self.rewardData.fancyCardPiece ~= nil then
        local fancyCardPieceModel = FancyCardPieceModel.new()
        local cardPieceData = self.rewardData.fancyCardPiece[1]
        fancyCardPieceModel:InitWithStatic(cardPieceData.id, cardPieceData.add)
        self:InstantiateFancyCardPieceBox(fancyCardPieceModel)
        table.remove(self.rewardData.fancyCardPiece, 1)
    end
...
function RewardBoxView:InstantiateAvatarPieceBox(cardPieceModel)
    ...
    self.rewardName = cardPieceModel:GetPieceName()
    self:SetName()
end
...
--实例化梦幻卡碎片
function RewardBoxView:InstantiateFancyCardPieceBox(fancyCardPieceModel)
    GameObjectHelper.FastSetActive(self.itemParentBox, true)
    GameObjectHelper.FastSetActive(self.cardParent.gameObject, false)
    local fancyCardPieceObj, fancyCardPieceView = res.Instantiate("Assets/CapstonesRes/Game/UI/Common/Part/FancyCardPiece.prefab")
    fancyCardPieceObj.transform:SetParent(self.itemParent, false)
    fancyCardPieceObj.transform.sizeDelta = Vector2(110, 140)
    fancyCardPieceView:InitView(fancyCardPieceModel, false, true, false)
    self.rewardName = fancyCardPieceModel:GetName()
    self:SetName()
end
