...
function PeakPlayerDetailView:ctor()
    ...
    -- 屏蔽玩家发言
    self.friendsBlockBtn = self.___ex.friendsBlockBtn
    self.friendsBlockBtnTxt = self.___ex.friendsBlockBtnTxt
    ...
    --全隱藏
    GameObjectHelper.FastSetActive(self.friendsBlockBtn.gameObject, false)
end
...
function PeakPlayerDetailView:start()
    ...
    self.friendsBlockBtn:regOnButtonClick(function()
        if type(self.onFriendsBlock) == 'function' then
            self.onFriendsBlock()
        end
    end)
...
function PeakPlayerDetailView:InitView(playerDetailModel, guildDetailModel, honorPalaceModel, sameGuild, isMe, showIndex, hideFunBtn)
    ...
    -- 屏蔽玩家发言
    self:SetFriendsBlockText(playerDetailModel:GetIsBlock())
...
-- 屏蔽玩家发言按钮
function PeakPlayerDetailView:SetFriendsBlockText(isBlock)
    if isBlock then
        self.friendsBlockBtnTxt.text = lang.transstr("friends_block_undo")
    else
        self.friendsBlockBtnTxt.text = lang.transstr("friends_block")
    end
end
