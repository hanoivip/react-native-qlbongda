...
--------Start_Auto_Generate--------
    ...
    self.bossBtn = self.___ex.bossBtn
    self.bossDisableGo = self.___ex.bossDisableGo
    self.bossRedPointGo = self.___ex.bossRedPointGo
--------End_Auto_Generate----------
    self.canvasGroup = self.___ex.canvasGroup
    self.commonTrans = self.___ex.commonTrans
    self.bossTrans = self.___ex.bossTrans
    self.mistTrans = self.___ex.mistTrans
end

function GuildWarTypeSelectView:start()
    -- 工会挑战赛暂时只有hk开 + KR
    if luaevt.trig('__HK__VERSION__') or luaevt.trig('__KR__VERSION__') then
        self.commonTrans.anchoredPosition = Vector2(-400, 0)
        self.mistTrans.anchoredPosition = Vector2(0, 0)
        GameObjectHelper.FastSetActive(self.bossTrans.gameObject, true)
    else
        self.commonTrans.anchoredPosition = Vector2(-200, 0)
        self.mistTrans.anchoredPosition = Vector2(200, 0)
        GameObjectHelper.FastSetActive(self.bossTrans.gameObject, false)
    end
    self:RegBtnEvent()
...
function GuildWarTypeSelectView:RegBtnEvent()
    ...
    self.bossBtn:regOnButtonClick(function()
        self:OnBtnBossClick()
    end)
end

function GuildWarTypeSelectView:InitView(guildWarTypeSelectModel)
    GameObjectHelper.FastSetActive(self.commonTrans.gameObject, true)
    GameObjectHelper.FastSetActive(self.mistTrans.gameObject, true)
    ...
    GameObjectHelper.FastSetActive(self.bossDisableGo, not self.model:GetBossIsOpen())
    GameObjectHelper.FastSetActive(self.bossBtn.gameObject, self.model:GetBossIsOpen())
    self:IsShowBossRedPoint()
end
...
-- 是否显示boss战红点
function GuildWarTypeSelectView:IsShowBossRedPoint()
    -- boss战没开时将事件次数置零
    if not self.model:GetBossIsOpen() then
        ReqEventModel.SetInfo("guildBossWar", 0)
    end
    local guildBossWar = ReqEventModel.GetInfo("guildBossWar")
    GameObjectHelper.FastSetActive(self.bossRedPointGo, tonumber(guildBossWar) > 0)
end
...
--点击世界BOSS
function GuildWarTypeSelectView:OnBtnBossClick()
    if self.onBtnBossClick then
        self.onBtnBossClick()
    end
end
