...
function MatchUI:onMatchEvent(frame)
    ...
    self.matchInfoModel:updateConditionsResultByEvent(stats.player.event, stats.opponent.event)
    if self.playerTeamData ~= nil and self.opponentTeamData ~= nil then
...
function MatchUI:onPenaltyShootOutStart()
    ...
    self.fightMenuManager:SetPanelActive(MatchConstants.CurrentUIPanel.SPECIAL_CONDITION, false)
...
function MatchUI:requestGameOver()
    ...
    local reqData = {
        ...
        statistics =
        {
            ...
            conditionsState = self.matchInfoModel:hasQuestConditions() and self.matchInfoModel:getConditionsResult() or nil,
        },
    ...
    local request = self:coroutine(function()
        ...
        if api.success(resp) then
            ...
            local playerScore = self.playerTeamData.stats.score
            local opponentScore = self.opponentTeamData.stats.score
            self.matchInfoModel:updateConditionsResultByEvent(statistics.player.event, statistics.opponent.event)
            ...
            elseif data.matchType == MatchConstants.MatchType.GUILDBOSSWAR then
                local guildBossMatchModel = require("ui.models.guild.guildBoss.GuildBossMatchModel").new()
                local playerTeamData = self.matchInfoModel:GetPlayerTeamData()
                local opponentTeamData = self.matchInfoModel:GetOpponentTeamData()
                data.playerInfo = {}
                data.opponentInfo = {}
                ...
                data.opponentInfo.logo = opponentTeamData.logo
                data.opponentInfo.teamName = opponentTeamData.teamName
                guildBossMatchModel:InitProtocolData(data)
            else
...
function MatchUI:onStartTime()
    ...
    self.fightMenuManager:SetPanelActive(MatchConstants.CurrentUIPanel.SPECIAL_CONDITION, true)
end
