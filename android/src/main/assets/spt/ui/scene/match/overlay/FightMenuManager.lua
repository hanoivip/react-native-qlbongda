...
function FightMenuManager:ctor()
    ...
    self.specialConditionGo = self.___ex.specialConditionGo
    ...
    self.specialConditionComponent = self.specialConditionGo:GetComponent(CapsUnityLuaBehav)
    self.fingerGesture = self.___ex.fingerGesture
...
function FightMenuManager:SetPanelActive(panel, isActive)
    ...
    elseif panel == MatchConstants.CurrentUIPanel.SPECIAL_CONDITION then
        self.specialConditionGo:SetActive(isActive)
    end
end
...
function FightMenuManager:onAthleteBuff(buff)
    ...
    if buff.State == 0 then
        ...
        if buff.SkillId == "A02" or buff.SkillId == "A07" or buff.SkillId == "A08"
            ...
            self:coroutine(function()
                coroutine.yield(UnityEngine.WaitForSecondsRealtime(0.9))
...
function FightMenuManager:StartPlayback()
    ...
    self:SetPanelActive(MatchConstants.CurrentUIPanel.SPECIAL_CONDITION, false)
    self:SetPanelActive(MatchConstants.CurrentUIPanel.PLAYER_NAME_PANEL, false)
end
    ...
    if MatchInfoModel.GetInstance():IsDemoMatch() then
        ...
        if not ___matchUI.inPenaltyShootOut then
            self:coroutine(function()
                ...
                self:SetPanelActive(MatchConstants.CurrentUIPanel.SPECIAL_CONDITION, true)
            end)
        end
    end
    self:SetPanelActive(MatchConstants.CurrentUIPanel.PLAYER_NAME_PANEL, true)
