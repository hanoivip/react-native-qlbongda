...
function RewardNameHelper.GetTypeName(rewardTable, rewardType)
    ...
    if rewardType == MailRewardType.FancyCard then -- 梦幻卡
        ...
        fancyCardModel:InitData(rewardTable.id)
        return fancyCardModel:GetName()
    end
    if rewardType == MailRewardType.FancyCardPiece then -- 梦幻卡碎片
        local fancyCardPieceModel = FancyCardPieceModel.new()
        fancyCardPieceModel:InitWithStatic(rewardTable.id)
        return fancyCardPieceModel:GetPieceName()
    end
    return ""
end
