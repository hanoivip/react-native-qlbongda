...
Tag.Card = "card"
Tag.Recycle = "recycle"
Tag.FancyCardPiece = "fancyCardPiece"
...
--------Start_Auto_Generate--------
    ...
    self.fancyPieceGo = self.___ex.fancyPieceGo
    self.pieceScrollSpt = self.___ex.pieceScrollSpt
    ...
    self.pieceInfoTxt = self.___ex.pieceInfoTxt
    self.pieceUseBtn = self.___ex.pieceUseBtn
    ...
    self.fancyPieceItemSpt = self.___ex.fancyPieceItemSpt
--------End_Auto_Generate----------
...
function FancyRecycleView:BindButtonHandler()
    ...
    -- 梦幻碎片合成
    self.pieceUseBtn:regOnButtonClick(function()
        self:OnPieceUseBtnClick()
    end)
end
...
function FancyRecycleView:InitView(fancyRecycleModel, fancyBagModel, fancyPieceBagModel, tag, fancyCardResourceCache)
    ...
    self.fancyPieceBagModel = fancyPieceBagModel
...
function FancyRecycleView:InitFancyPiece()
    local pieceModelList = self.fancyPieceBagModel:GetFilterPieceModelList()
    self.pieceScrollSpt:InitView(pieceModelList, function(fancyPieceModel) self:OnFancyPieceClick(fancyPieceModel) end)
    local fancyPieceModel = self.fancyPieceBagModel:GetSelectCardPieceModel()
    self:InitFancyPieceInfo(fancyPieceModel)
end

function FancyRecycleView:OnFancyPieceClick(fancyPieceModel)
    self.fancyPieceBagModel:SetSelectCardPieceModel(fancyPieceModel)
    local pieceModelList = self.fancyPieceBagModel:GetFilterPieceModelList()
    GameObjectHelper.FastSetActive(self.infoGo, tobool(fancyPieceModel))
    local scrollPos = self.pieceScrollSpt:GetScrollNormalizedPosition()
    scrollPos = math.clamp(scrollPos, 0, 1)
    self.pieceScrollSpt:refresh(pieceModelList)
    self.pieceScrollSpt:SetScrollNormalizedPosition(scrollPos)
    self:InitFancyPieceInfo(fancyPieceModel)
end

function FancyRecycleView:InitFancyPieceInfo(fancyPieceModel)
    GameObjectHelper.FastSetActive(self.infoGo, tobool(fancyPieceModel))
    if fancyPieceModel then
        self.fancyPieceItemSpt:InitView(fancyPieceModel)
        local pieceName = fancyPieceModel:GetName()
        local pieceNum = fancyPieceModel:GetNum()
        local pieceDesc = fancyPieceModel:GetDesc()
        self.pieceNameTxt.text = pieceName
        self.pieceNumTxt.text = lang.trans("has_piece_num", pieceNum)
        self.pieceInfoTxt.text = pieceDesc
    end
end
...
function FancyRecycleView:OnPieceUseBtnClick()
    if self.pieceUse then
        self.pieceUse()
    end
end
