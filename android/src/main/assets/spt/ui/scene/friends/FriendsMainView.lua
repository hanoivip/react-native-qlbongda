...
function FriendsMainView:ctor()
    ...
    -- 屏蔽玩家发言
    self.friendsBlockBoard = self.___ex.friendsBlockBoard
end
...
function FriendsMainView:start()
    ...
    for i = 1, menuTransform.childCount do
        ...
        btnObject:GetComponent("CapsUnityLuaBehav"):regOnButtonClick(function()
            self:OnMenuClick(i)
        end)
    end
...
function FriendsMainView:SwitchFriendsTab(model)
    ...
    if menuType == MenuType.MESSAGES then
        self:ShowOrHideFriendsBoard(true, false, false, false, false)
    elseif menuType == MenuType.MANAGER then
        self:ShowOrHideFriendsBoard(false, true, false, false, false)
    elseif menuType == MenuType.ADD then
        self:ShowOrHideFriendsBoard(false, false, true, false, false)
    elseif menuType == MenuType.INVITE then
        self:ShowOrHideFriendsBoard(false, false, false, true, false)
    elseif menuType == MenuType.BLOCK then
        self:ShowOrHideFriendsBoard(false, false, false, false, true)
    end
end

function FriendsMainView:ShowOrHideFriendsBoard(isShowMessageBoard, isShowManagerBoard, isShowAddBoard, isShowInviteBoard, isShowBlockBoard)
    ...
    self.friendsBlockBoard:SetActive(isShowBlockBoard)
end
