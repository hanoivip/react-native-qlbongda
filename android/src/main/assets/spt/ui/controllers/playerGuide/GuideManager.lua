...
function GuideManager.ShowPlayerGuide()
    ...
    if guideModel:GetCurModule() == "main" and step == "1100" then
        local PlayerInfoModel = require("ui.models.PlayerInfoModel")
        local playerInfoModel = PlayerInfoModel.new()
        local aftLvl = playerInfoModel.GetLevel()
        ...
        luaevt.trig("SDK_Report", "guide_begin", roleId, aftLvl, roleName, serverCode, serverName)
    end
    return objs
end
...
function GuideManager.LevelGuide()
    ...
    if level == LevelLimit.Coach.playerLevel then
    ...
    if luaevt.trig('__HK__VERSION__') then
        return
    end
    if level == 4 then
