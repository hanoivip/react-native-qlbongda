...
local FriendsBlockCtrl = require("ui.controllers.friends.FriendsBlockCtrl")
...
function FriendsMainCtrl:Init(currentMenu, friendsMainModel)
    ...
    -- 屏蔽玩家发言
    self.friendsBlockCtrl = FriendsBlockCtrl.new(res.GetLuaScript(self.view:GetFriendsBoard(MenuType.BLOCK)))
    self.friendsPageMap = {}
end
...
function FriendsMainCtrl:OnEnterScene()
    ...
    self.friendsBlockCtrl:OnEnterScene()
...
function FriendsMainCtrl:OnExitScene()
    ...
    self.friendsBlockCtrl:OnExitScene()
    for k, v in pairs(self.friendsPageMap) do
        v:OnExitScene()
    end
end
...
function FriendsMainCtrl:OnMenuClick(index)
    ...
    elseif index == MenuType.BLOCK then
        self:OnFriendsBlock()
    end
...
function FriendsMainCtrl:OnFriendsManager()
    clr.coroutine(function()
        local response = req.friendsListFriends()
        if api.success(response) then
            local friendsManagerModel = self.friendsMainModel:GetFriendsManagerModel()
            friendsManagerModel:InitWithProtocol(response.val)
...
function FriendsMainCtrl:OnFriendsAdd()
    clr.coroutine(function()
        local response = req.friendsFind("", PlayerInfoModel.new():GetSID())
        if api.success(response) then
            local friendsAddModel = self.friendsMainModel:GetFriendsAddModel()
            friendsAddModel:InitWithProtocol(response.val)
...
function FriendsMainCtrl:OnFriendsInvite()
    self.view:coroutine(function()
        local response = req.fiTaskInfo(nil, nil, true)
        if api.success(response) then
...
-- 屏蔽玩家发言
function FriendsMainCtrl:OnFriendsBlock()
    clr.coroutine(function()
        local response = req.friendsBlockList()
        if api.success(response) then
            local friendsBlockModel = self.friendsMainModel:GetFriendsBlockModel()
            friendsBlockModel:InitWithProtocol(response.val)
            self.friendsBlockCtrl:InitView(friendsBlockModel)
        end
    end)
end
