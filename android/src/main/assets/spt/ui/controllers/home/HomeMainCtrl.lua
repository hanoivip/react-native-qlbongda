...
function HomeMainCtrl:RefreshHomeEvent()
    self.view:coroutine(function()
        ...
        if api.success(response) then
            ...
            if data.eventGiftBag then
                local model = EventGiftBoxModel.new()
                model:InitWithProtocol(data.eventGiftBag)
...
function HomeMainCtrl:Refresh()
    ...
    self:ShowOtherGuide()
    if not GuideManager.HasGuideOnGoing() or GuideManager.GuideIsOnGoing('beginnerCarnival') then
...
function HomeMainCtrl:Init()
    ...
    self.view.clickCoach = function() self:OnBtnCoach() end
    self.view.clickFreshPlayerLevel = function() self:OnBtnFreshPlayerLevel() end
    self.view.homeCourtUpdate = function() self:OnHomeCourtUpdate() end
end

function HomeMainCtrl:OnHomeCourtUpdate()
    self.homeFirstTeamCtrl:OnHomeCourtUpdate()
end
...
function HomeMainCtrl:ShowOtherGuide()
    self:ShowCoachGuide()
    self:ShowFancyGuide()
end

function HomeMainCtrl:ShowCoachGuide()
    -- Coach Guide  在所有引导之后， 根据等级开启引导
    local coachOpenLevel = LevelLimit.Coach.playerLevel
    local level = self.playerInfoModel:GetLevel()
    local coachOpenState = coachOpenLevel <= level
    local hasGuideOnGoing = GuideManager.HasGuideOnGoing()
    local coachGuideState = coachOpenState and (not hasGuideOnGoing)
    if coachGuideState then
        GuideManager.InitCurModule("coach")
        GuideManager.Show(self)
    end
end

function HomeMainCtrl:ShowFancyGuide()
    -- Fancy Guide  在所有引导之后， 根据等级开启引导
    if not LevelLimit.Fancy then
        return
    end
    local fancyOpenLevel = LevelLimit.Fancy.playerLevel
    local level = self.playerInfoModel:GetLevel()
    local fancyOpenState = fancyOpenLevel <= level
    local hasGuideOnGoing = GuideManager.HasGuideOnGoing()
    local fancyGuideState = fancyOpenState and (not hasGuideOnGoing)
    if fancyGuideState then
