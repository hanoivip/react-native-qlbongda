...
function PlayerDetailCtrl:InitView()
    ...
    self.initialBlockState = self.playerDetailModel:GetIsBlock()
    self.view.onClose = function()
        if not self.changeBlock or not self.initialBlockState then
            return
        end
        local isBlock = self.playerDetailModel:GetIsBlock()
        if isBlock then
            EventSystem.SendEvent("FriendsBlock_Update")
        else
            EventSystem.SendEvent("FriendsBlock_Remove", self.pid)
        end
    end
...
-- 屏蔽玩家发言
function PlayerDetailCtrl:OnFriendsBlock()
