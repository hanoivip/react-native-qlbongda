    ...
    for i, pcid in ipairs(cardList) do
        ...
        if not cidSelect[cid] then
            if ascend < maxAscend then
                ...
                if OnlyTargetPlayer[fixedQuality] then
                    cidSelect[cid] = tmpCardModel
                end
            end
        end
    end
    for i, v in pairs(cidSelect) do
        if not maxAscendList[i] then
            table.insert(cardModelList, v)
        end
    end
