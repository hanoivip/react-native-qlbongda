...
function CardIndexCtrl:RegScrollComp()
    ...
    self.view.scrollView.onScrollResetItem = function(spt, index)
        ...
        local showCustomTag = false
        if luaevt.trig('__HK__VERSION__') or luaevt.trig('__VN__VERSION__') then
            showCustomTag = true
        end
        spt:InitView(itemData, MenuType.LIST, self.cardIndexModel, self.cardResourceCache, self, showCustomTag, customTagModel)
